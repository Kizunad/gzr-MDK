...
public class GuzhenrenDevKitExampleMod {
	...
	private static final LianGuInputMode INPUT_MODE = LianGuInputMode.SLOT0_ONLY;
		...
		// 炼蛊进度条机制：主模组进度文本显示“炼制进度:X/100”。
		// 需求（来自测试反馈）：
		// 1) 进度必须 1 个 1 个地涨（step=1），而不是一次跳 10。
		// 2) 进度涨到 100/100 后必须立刻产出泥土，不能卡在 100 不动。
		// 实现：每 tick 进度 +1（20 tick/秒 → 约 5 秒填满 100），达到满值即产出。
		private static final double LIANGU_MAX = 100d;                       // 进度条满值
	...
	public GuzhenrenDevKitExampleMod() {
		...
		ExampleNiXiangLianGu.bootstrap(MODID);
		bootstrapLianGu();
	}
	...
	private static void bootstrapLianGu() {
		...
		LianGuRecipes.register(new ExternalLianGuRecipe() {
			...
			public void onTick(Player player) {
				...
				if (vars.LianGuJinDu < LIANGU_MAX) {
					vars.LianGuJinDu = vars.LianGuJinDu + LIANGU_STEP;
					if (vars.LianGuJinDu > LIANGU_MAX) {
						vars.LianGuJinDu = LIANGU_MAX; // 防浮点误差越界
					}
				}

				// 进度达到满值（100/100）立即产出泥土。
				if (vars.LianGuJinDu >= LIANGU_PRODUCE_AT) {
					vars.LianGuJinDu = 0;
					vars.GuFang = 0;
					vars.LianGu = false;
					vars.markSyncDirty();
					...
					if (!player.level().isClientSide()) {
						LOGGER.info("[{}] liangu finish ok: produced 1x DIRT (进度条已满)", MODID);
					}
					return;
				}
